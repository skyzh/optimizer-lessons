        ...
        while let Some((merge_into, merge_from)) = merge_worklist.pop() {
            ...
            let mut affected = self
                ...
                .filter_map(|(expr_id, expr)| {
                    expr.children.contains(&merge_from).then_some(*expr_id)
                })
                .collect::<Vec<ExprId>>();
            affected.sort();
            ...
            // Process collisions in the same depth-first order as the recursive
            // algorithm, but keep the pending work on the heap instead of the
            // call stack.
